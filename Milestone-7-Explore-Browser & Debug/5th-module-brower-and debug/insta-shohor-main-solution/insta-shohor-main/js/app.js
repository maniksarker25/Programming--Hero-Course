...
const getReportedPosts = () => {
  // console.log(reportedPostsId)
    const report =  posts.filter((post) => reportedPostsId.includes(post.id));
    console.log(report);
    return report;
};
...
const addToLiked = (id) => {
  ...
  if(likedPostsId.includes(id)){
    likedPostsId = likedPostsId.filter(item => item != id)
    // likedPostsId.pop();
    showPosts(posts)
  }
  else{
...
const reportPost = (id) => {
    if(reportedPostsId.includes(id)){
      reportedPostsId = reportedPostsId.filter(item => item != id);
